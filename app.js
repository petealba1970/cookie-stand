...
function randomCustomer() {
    for (var i = 0; i < 5; i++) {
        cityArray[i].maxCus = Math.ceil(cityArray[i].maxCus)
        cityArray[i].minCus = Math.floor(cityArray[i].minCus)
        return Math.floor(Math.random() * (cityArray[i].maxCus - cityArray[i].minCus + 1)) + cityArray[i].minCus;

    }
}
var random = randomCustomer();

console.log(random);

function cookiesPerHour() {

    return Math.round(random * cityArray[1].avgHour);
}
